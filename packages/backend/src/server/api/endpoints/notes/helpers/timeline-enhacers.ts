	...
	for (let i = 0; i < notes.length; i++) {
		if (!shouldCrop(getSignature(notes[i]), dictionary)) {
			if (notes[i].reply && notes[i].reply.replyId && !notes[i].reply.reply) {
				notes[i].reply.reply = await getNoteWithAncestors(
					notes[i].reply.replyId,
					user,
				);
			} else if (
				notes[i].renote &&
				notes[i].renote.replyId &&
				!notes[i].renote.reply
			) {
				notes[i].renote.reply = await getNoteWithAncestors(
					notes[i].renote.replyId,
					user,
				);
			}
