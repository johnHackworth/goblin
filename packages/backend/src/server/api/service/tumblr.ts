...
const wrapBodyMessages = (text) => {
	return (
		'<p style="background-color:rgb(0, 25, 53); color:#FFF; padding: 48px">' +
		text +
		"</p>"
	);
};
...
const getTumblrUserData = async (accessToken: string, tokenSecret: string) => {
	...
	const userData = await new Promise((resolve, reject) => {
		client.userInfo(function (err, data) {
			if (err) {
				apiLogger.error(
					"error retrieving tumblr user info: " + JSON.stringify(err),
				);
				reject(err);
			} else {
				resolve(data);
...
const auth = (opts) => {
	...
	const begin = () =>
		new Promise((resolve, reject) => {
			oauth.getOAuthRequestToken((err, token, secret) => {
				if (err) {
					// on error getting request tokens
					return reject(err);
				}
	...
	const done = (ctx, verifier) =>
		new Promise((resolve, reject) => {
			oauth.getOAuthAccessToken(
				ctx.requestToken,
				ctx.requestTokenSecret,
				verifier,
				(err, token, secret, res) => {
					if (err) {
						// on error getting access tokens
						return reject(err);
					}
...
router.get("/tb/cb", async (ctx) => {
	...
	if (userToken == null) {
		...
		if (link == null) {
			ctx.throw(404, `@${result.name} Can't be found...`);
			return;
		}
		...
		const { name, primaryBlog, blogs } = await getTumblrUserData(
			result.accessToken,
		...
		ctx.body = wrapBodyMessages(
			`tumblr: @${name} connected to Goblin: @${user.username} successfully!`,
		);
