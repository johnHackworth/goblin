<template>
	<article class="_panel tag-card">
		<div
			class="banner"
			:style="
				tag?.bannerUrl ? `background-image: url(${user.bannerUrl})` : ''
			"
			:class="{ detailed }"
		>
			<span
				v-if="props.isFollowed"
				class="followed"
		...
		<div class="status">
			<p>
				<MkNumber :value="props.tag.attachedUserIds" />
				{{ i18n.ts.notes }}
			</p>
			<p>
		...
		<div class="buttons">
			<slot>
				<MkFollowHashtagButton :tag="props.tag.name" />
				<MkBlockHashtagButton :tag="props.tag.name" />
			</slot>
		</div>
	</article>
</template>
